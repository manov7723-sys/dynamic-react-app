...
# 2 subnet associations · certificate auth · full-tunnel · UDP/443 · certs: auto
...
resource "aws_security_group" "vpn" {
  ...
  description = "Security group attached to the client-vpn Client VPN endpoint. Reference this from RDS/EC2 SGs' ingress rules."
...
resource "aws_ec2_client_vpn_network_association" "assoc_0" {
  ...
  subnet_id              = "subnet-0589bda4e647c5268"
}

resource "aws_ec2_client_vpn_network_association" "assoc_1" {
  client_vpn_endpoint_id = aws_ec2_client_vpn_endpoint.this.id
  subnet_id              = "subnet-0935fed30dcb57731"
}
...
resource "aws_ec2_client_vpn_route" "internet" {
  ...
  target_vpc_subnet_id   = "subnet-0589bda4e647c5268"
